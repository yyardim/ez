define('filter.eventz',
    ...
    function (ko, utils, config) {
        ...
        var EventFilter = function () {
            ...
            self.eventerz = ko.observable(); // object
        ...
        EventFilter.prototype = function () {
            var tagDelimiter = '|',
                escapedTagDelimiter = '\\|',
                searchTest = function (searchText, event) {
                    if (!searchText) return true; // always succeeds if no search text
                    var srch = utils.regExEscape(searchText.toLowerCase());
                    if (event.name().toLowerCase().search(srch) !== -1) return true;
                    if (event.eventerz().firstName().toLowerCase().search(srch) !== -1) return true;
                    if (event.eventerz().lastName().toLowerCase().search(srch) !== -1) return true;
                    //if (session.track().name().toLowerCase().search(srch) !== -1) return true;
                    //if (session.room().name().toLowerCase().search(srch) !== -1) return true;
                    if ((tagDelimiter + event.tags().toLowerCase() + tagDelimiter)
                ...
                predicate = function (self, event) {
                    ...
                    var match = searchTest(self.searchText(), event);
